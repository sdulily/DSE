...
class BaseDataset(data.Dataset):
    ...
    def initialize(self, opt, k=None):
        pass
...
def transform_few_with_label(opt, A, B, C, label, Shapes, Colors, Colors_CA):
    if not opt.resize_or_crop == 'none':
        raise ValueError(
            "Only support none mode for resize_or_crop on base_gray_color dataset")
    assert(isinstance(Shapes, list))
    assert(isinstance(Colors, list))
    assert (isinstance(Colors_CA, list))
    ...
    Colors = list(map(lambda c: transforms.Normalize(
        (0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(transforms.ToTensor()(c)), Colors))
    Colors_CA = list(map(lambda ca: transforms.Normalize(
        (0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(transforms.ToTensor()(ca)), Colors_CA))

    Shapes = torch.cat(Shapes)
    Colors = torch.cat(Colors)
    Colors_CA = torch.cat(Colors_CA)

    return A, B, C, label, Shapes, Colors, Colors_CA
