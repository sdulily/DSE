...
def define_G(input_nc, output_nc, nz, ngf, nencode=4, netG='unet_128', use_spectral_norm=False,
             ...
             init_type='xavier', gpu_ids=[], where_add='input', upsample='bilinear', wo_skip=False, cross=None):
    net = None
    ...
    if netG == 'agisnet':
        ...
        net = AGISNet(input_content, input_style, output_nc, 6, ngf,
                      ...
                      use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip=wo_skip, cross=cross)
...
class AGISNetBlock(nn.Module):
    def __init__(self, input_cont, input_style, outer_nc, inner_nc,
                 submodule=None, outermost=False, innermost=False, use_spectral_norm=False,
                 norm_layer=None, nl_layer=None, use_dropout=False, use_attention=False,
                 upsample='basic', padding_type='zero', wo_skip=False, cross=None):
        ...
        if input_style == 12:
            ...
            downconv3 += [nn.Conv2d(input_style, inner_nc, kernel_size=3, stride=2, padding=p)]
        else:
            downconv1 += [nn.Conv2d(input_cont, inner_nc, kernel_size=3, stride=2, padding=p)]
            downconv2 += [nn.Conv2d(input_style, inner_nc, kernel_size=3, stride=2, padding=p)]
            downconv3 += [nn.Conv2d(input_style, inner_nc, kernel_size=3, stride=2, padding=p)]
        ...
        downrelu3 = nn.LeakyReLU(0.2, True)
        uprelu = nl_layer()
        ...
        if outermost:
            if self.wo_skip:
                ...
                if self.cross == 0:
                    ...
                    upconv = upsampleLayer(
                        inner_nc * 3, inner_nc, upsample=upsample, padding_type=padding_type,
                        use_spectral_norm=use_spectral_norm)

            upconv_out = [nn.Conv2d(inner_nc, outer_nc, kernel_size=3, stride=1, padding=p)]
    ...
    def forward(self, content, shape, style, style_ca):

        x1 = self.down1(content)
        x2 = self.down2(shape)
        x3 = self.down3(style)
        x4 = self.down3(style_ca)
        if self.outermost:
            mid_C, x3, x4 = self.submodule(x1, x2, x3, x4)
            mid_C2 = self.up(mid_C)
            fake_C = self.up_out(mid_C2)
            return fake_C, x3, x4
        elif self.innermost:
            mid_C = torch.cat([x1, x2, x3], 1)
            fake_C = self.up(mid_C)
            if self.cross == 0:
                tmp1 = torch.cat([content, shape, style], 1)
            else:
                tmp1 = torch.cat([content, shape], 1)
            if self.wo_skip:
                return fake_C, x3, x4
            else:
                return torch.cat([fake_C, tmp1], 1), x3, x4
        else:
            mid, x3, x4 = self.submodule(x1, x2, x3, x4)
            fake_C = self.up(mid)
            if self.cross == 0:
                tmp1 = torch.cat([content, shape, style], 1)
            elif self.cross == 1:
...
class AGISNet(nn.Module):
    ...
    def __init__(self, input_content, input_style, output_nc, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False,
                 use_attention=False, use_spectral_norm=False, upsample='basic', wo_skip=False, cross=None):
        ...
        dual_block = AGISNetBlock(ngf*max_nchn, ngf*max_nchn, ngf*max_nchn, ngf*max_nchn,
                                  ...
                                  norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, wo_skip=wo_skip, cross=cross[0])
        for i in range(num_downs - 5):
            dual_block = AGISNetBlock(ngf*max_nchn, ngf*max_nchn, ngf*max_nchn, ngf*max_nchn, dual_block,
                                      norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout,
                                      use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip=wo_skip, cross=cross[1])
        dual_block = AGISNetBlock(ngf*4, ngf*4, ngf*4, ngf*max_nchn, dual_block, use_attention=use_attention,
                                  use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                  nl_layer=nl_layer, upsample=upsample, wo_skip=wo_skip, cross=cross[2])
        dual_block = AGISNetBlock(ngf*2, ngf*2, ngf*2, ngf*4, dual_block, use_attention=use_attention,
                                  use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                  nl_layer=nl_layer, upsample=upsample, wo_skip=wo_skip, cross=cross[3])
        dual_block = AGISNetBlock(ngf, ngf, ngf, ngf*2, dual_block, use_attention=use_attention,
                                  use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                  nl_layer=nl_layer, upsample=upsample, wo_skip=wo_skip, cross=cross[4])
        dual_block = AGISNetBlock(input_content, input_style, output_nc, ngf, dual_block,
                                  use_spectral_norm=use_spectral_norm, outermost=True, norm_layer=norm_layer,
                                  nl_layer=nl_layer, upsample=upsample, wo_skip=wo_skip, cross=cross[5])

        self.model = dual_block

    def forward(self, content, shape, style, style_ca):
        return self.model(content, shape, style, style_ca)
